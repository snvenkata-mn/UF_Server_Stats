...
/* OCILogon2 */
#if defined RUNTIME_API_CHECK
typedef sword (*oci8_OCILogon2_func_t)(OCIEnv *envhp, OCIError *errhp, OCISvcCtx **svchp, CONST text *username, ub4 uname_len, CONST text *password, ub4 passwd_len, CONST text *dbname, ub4 dbname_len, ub4 mode);
static oci8_OCILogon2_func_t oci8_OCILogon2_func;
#define OCILogon2 oci8_OCILogon2_func
#elif ORACLE_CLIENT_VERSION < ORAVER_9_0
#define OCILogon2(envhp, errhp, svchp, username, uname_len, password, passwd_len, dbname, dbname_len, mode) (0)
#endif
 ...
 */
#if defined RUNTIME_API_CHECK
int oci8_have_OCILogoff;
#endif

sword oci8_OCILogoff(OCISvcCtx *svchp, OCIError *errhp, const char *file, int line)
{
    if (have_OCILogoff) {
        return OCILogoff(svchp, errhp);
    } else {
        rb_raise(rb_eRuntimeError, "undefined OCI function %s is called", "OCILogoff");
    }
}
...
typedef struct {
    ...
    ub4 dbname_len;
    ub4 mode;
} oci8_OCILogon2_data_t;

#if defined RUNTIME_API_CHECK
int oci8_have_OCILogon2_nb;
#endif

#if defined RUNTIME_API_CHECK || ORACLE_CLIENT_VERSION >= ORAVER_9_0
static void *oci8_OCILogon2_cb(void *user_data)
{
    oci8_OCILogon2_data_t *data = (oci8_OCILogon2_data_t *)user_data;
    data->rv = OCILogon2(data->envhp, data->errhp, data->svchp, data->username, data->uname_len, data->password, data->passwd_len, data->dbname, data->dbname_len, data->mode);
...
#else
#define oci8_OCILogon2_cb NULL
#endif

sword oci8_OCILogon2_nb(oci8_svcctx_t *svcctx, OCIEnv *envhp, OCIError *errhp, OCISvcCtx **svchp, CONST text *username, ub4 uname_len, CONST text *password, ub4 passwd_len, CONST text *dbname, ub4 dbname_len, ub4 mode, const char *file, int line)
{
    if (have_OCILogon2_nb) {
        oci8_OCILogon2_data_t data;
        data.svcctx = svcctx;
        data.envhp = envhp;
        data.errhp = errhp;
        data.svchp = svchp;
        ...
        data.mode = mode;
        oci8_call_without_gvl(svcctx, oci8_OCILogon2_cb, &data);
        return data.rv;
    } else {
        rb_raise(rb_eRuntimeError, "undefined OCI function %s is called", "OCILogon2_nb");
    }
}
...
void Init_oci8_apiwrap(void)
{
    ...
    if (oci8_OCIIntervalSetYearMonth_func == NULL)
        return;
    oci8_OCILogon2_func = (oci8_OCILogon2_func_t)oci8_find_symbol("OCILogon2");
    if (oci8_OCILogon2_func == NULL)
        return;
